...
```
 ...
 CAS（等待期间消耗CPU，线程太多，带来的上下文切换，成本高，使CPU资源一直浪费在线程上下文切换中）
...
```
不一定，在明知会存在多线程同时执行的情况下（JVM启动过程中，内部有多线程），若启用偏向锁，会涉及到偏向锁撤销，再升级为轻量级锁的过程。因此，jvm在启动时默认情况下是关闭偏向锁的，等启动完成后再启用偏向锁
```
